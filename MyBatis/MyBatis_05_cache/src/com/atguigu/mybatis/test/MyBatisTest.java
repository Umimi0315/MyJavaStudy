...
public class MyBatisTest {
     ...
     *      2.如果会话关闭；一级缓存中的 数据会被保存到二级缓存中，新的会话查询信息，就可以参照二级缓存中的内容
     ...
     *      3）、【每个增删改标签的flushCache="true":(一级、二级都会清除)】
     *                  增删改执行完成后就会清除缓存
     *                  测试：flushCache="true":一级缓存就清空了，二级缓存也会被清除
     *                  查询标签：flushCache="false":
     *                      如果flushCache="true":每次查询之后都会清除缓存，缓存是没有被使用的
...
//            EmployeeMapper mapper2 = openSession2.getMapper(EmployeeMapper.class);
            ...
            //3、sqlSession相同，两次查询之间执行了增删改操作（这次增删改可能对当前数据有影响）
//            mapper.addEmp(new Employee(null,"testCache","cache@guigu.com","1"));
...
//            mapper2.addEmp(new Employee(null,"aaa","nnn","0"));
    ...
    @Test
    public void testSecondLevelCache02() throws Exception {
        SqlSessionFactory sqlSessionFactory = getSqlSessionFactory();
        SqlSession openSession = sqlSessionFactory.openSession();
        SqlSession openSession2 = sqlSessionFactory.openSession();
        try {
